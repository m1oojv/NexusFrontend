...
function ClientDetails() {
  ...
  isCompleted = checkCompleted(companyDetails.assessment_progress);

  const link = useQuery(
    "generatepdf",
    () => api_reports.getPdfReport({ id: companyDetails.id }),
    {
  ...
  return (
    <React.Fragment>
      ...
      <Grid container spacing={3} style={{ display: "flex" }}>
        ...
        <Grid item xs={12} sm={12} md={12} lg={6}>
          <ClickablePanel
            data={companyDetails.scan_results.data.credentials}
            title="Exposed Credentials"
            description={ExposedCredentialsDescription}
            records={
              companyDetails.scan_results.data.credentials.total_creds_exposed
            }
            score={companyDetails.scan_results.risk_results.creds_score}
            icon={<VpnKeyOutlinedIcon style={{ fontSize: 18 }} />}
          />
        </Grid>
        <Grid item xs={12} sm={12} md={12} lg={6}>
          <ClickablePanel
            data={companyDetails.scan_results.data.email_security}
            title="Email Security"
            description={EmailSecurityDescription}
            records={2}
            score={
              companyDetails.scan_results.risk_results.email_security_score
            }
        ...
        <Grid item xs={12} sm={12} md={12} lg={6}>
          <ClickablePanel
            data={companyDetails.scan_results.data.marketplace}
            title="Marketplace Mentions"
            description={CredsOnMarketDescription}
            records={companyDetails.scan_results.data.marketplace.length}
            score={companyDetails.scan_results.risk_results.marketplace_score}
            icon={<PublicOutlinedIcon style={{ fontSize: 18 }} />}
          />
        </Grid>
        <Grid item xs={12} sm={12} md={12} lg={6}>
          <ClickablePanel
            data={companyDetails.scan_results.data.vulns}
            title="Vulnerabilities"
            description={VulnerabilitiesDescription}
            records={companyDetails.scan_results.data.vulns.totalNumberOfVulns}
            score={companyDetails.scan_results.risk_results.vulns_score}
            icon={<ReportOutlinedIcon style={{ fontSize: 18 }} />}
          />
        </Grid>
        <Grid item xs={12} sm={12} md={12} lg={6}>
          <ClickablePanel
            data={companyDetails.scan_results.data.ports}
            title="Services"
            description={ServicesDescription}
            records={
              companyDetails.scan_results.data.ports.totalNumberOfHighRisk +
              companyDetails.scan_results.data.ports.totalNumberOfMediumRisk
            }
            score={companyDetails.scan_results.risk_results.ports_score}
